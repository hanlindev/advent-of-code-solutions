...
class GridPanel {
    ...
    public getColor(location: Coordinates): number {
        return (
            this.grid.get(this.getLocationKey(location)) ??
            this.getDefaultColor(location)
        );
    }
    private getDefaultColor(location: Coordinates): number {
    ...
    private updateBoundaries(location: Coordinates) {
        this.minX = Math.min(location.x, this.minX);
        this.maxX = Math.max(location.x, this.maxX);

        this.minY = Math.min(location.y, this.minY);
        this.maxY = Math.max(location.y, this.maxY);
    }
    ...
    public showPainting() {
        ...
        for (let y = this.maxY; y >= this.minY; --y) {
            ...
            for (let x = this.minX; x <= this.maxX; ++x) {
                const current = new Coordinates(x, y);
                row.push(this.getColor(current) === COLOR_WHITE ? "#" : " ");
            }
